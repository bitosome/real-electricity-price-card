 ...
 */class ct extends b{constructor(){super(...arguments),this.renderOptions={host:this},this._$Do=void 0}createRenderRoot(){const t=super.createRenderRoot();return this.renderOptions.renderBefore??=t.firstChild,t}update(t){const e=this.render();this.hasUpdated||(this.renderOptions.isConnected=this.isConnected),super.update(t),this._$Do=((t,e,r)=>{const i=r?.renderBefore??e;let s=i._$litPart$;if(void 0===s){const t=r?.renderBefore??null;i._$litPart$=s=new tt(e.insertBefore(F(),t),t,void 0,r??{})}return s._$AI(t),s})(e,this.renderRoot,this.renderOptions)}connectedCallback(){super.connectedCallback(),this._$Do?.setConnected(!0)}disconnectedCallback(){super.disconnectedCallback(),this._$Do?.setConnected(!1)}render(){return W}}ct._$litElement$=!0,ct.finalized=!0,at.litElementHydrateSupport?.({LitElement:ct});const lt=at.litElementPolyfillSupport;lt?.({LitElement:ct}),(at.litElementVersions??=[]).push("4.2.2");const ht="sensor.real_electricity_price_chart_data",dt="sensor.real_electricity_price_current_price",pt="Electricity Price",ut="€/kWh",mt={"#bfdbfe":"past_color","#3b82f6":"current_hour_color","#93c5fd":"future_color","#bbf7d0":"cheap_past_color","#86efac":"cheap_color","#22c55e":"cheap_current_color"};function _t(t,e,r){t.dispatchEvent(new CustomEvent(e,{detail:r,bubbles:!0,composed:!0}))}function ft(t,e,r){const i=Number(t);if(Number.isFinite(i))return Math.max(e,Math.min(r,i))}function gt(t){if("string"!=typeof t)return;return t.trim()||void 0}function $t(t){if("number"==typeof t&&Number.isFinite(t))return t;if("string"==typeof t){const e=Number(t);if(Number.isFinite(e))return e;const r=new Date(t).getTime();if(Number.isFinite(r))return r}}function xt(t){return"line"===(t.chart_type||t.graph_type)?"line":"bar"}function yt(t){return ft(t.height,120,360)||190}function bt(t){return Math.round(ft(t.horizontal_lines,2,9)||5)}function vt(t){return Math.round(ft(t.price_decimals,0,6)??4)}function At(t){return Math.round(ft(t.axis_decimals,0,4)??2)}function wt(t,e,r=vt(e)){return void 0!==t&&Number.isFinite(t)?`${t.toFixed(r)} ${e.unit||ut}`:"—"}function Et(t){const e={hour:"2-digit",minute:"2-digit"},r=String(t?.locale?.time_format||"").toLowerCase();return"12"===r&&(e.hour12=!0),"24"===r&&(e.hour12=!1),e}function Ct(t,e){return new Intl.DateTimeFormat(t?.locale?.language,{day:"numeric",month:"long",...Et(t)}).format(new Date(e))}function St(t){const e=t.slice(1).map((e,r)=>e.timestamp-t[r].timestamp).filter(t=>Number.isFinite(t)&&t>0).sort((t,e)=>t-e);return e.length&&e[Math.floor(e.length/2)]||36e5}function kt(t,e,r,i){const s=r.width-r.left-r.right;if(e.length<=1)return r.left+s/2;const o=St(e),n=e[0].timestamp,a=e[e.length-1].timestamp,c="bar"===i?n-o/2:n,l="bar"===i?a+o/2:a,h=Math.max(1,l-c);return r.left+(t-c)/h*s}function Mt(t){if(!t.length)return"";if(1===t.length)return`M ${t[0].x.toFixed(2)} ${t[0].y.toFixed(2)}`;let e=`M ${t[0].x.toFixed(2)} ${t[0].y.toFixed(2)}`;for(let r=0;r<t.length-1;r+=1){const i=t[r],s=t[r+1],o=t[r-1]||i,n=t[r+2]||s,a=i.x+(s.x-o.x)/6,c=i.y+(s.y-o.y)/6,l=s.x-(n.x-i.x)/6,h=s.y-(n.y-i.y)/6;e+=` C ${a.toFixed(2)} ${c.toFixed(2)}, ${l.toFixed(2)} ${h.toFixed(2)}, ${s.x.toFixed(2)} ${s.y.toFixed(2)}`}return e}function Pt(t,e){const r=(i=t.color)?i.trim().toLowerCase():void 0;var i;const s=r?e.color_overrides?.[r]||e.color_overrides?.[t.color||""]:void 0;if(s)return s;const o=r?mt[r]:void 0;return o&&gt(e[o])?String(e[o]):!1!==e.use_sensor_colors&&t.color?t.color:e.line_color||"#ffc928"}function Ft(t){return{entity:ht,current_price_entity:dt,name:pt,chart_type:"bar",height:190,horizontal_lines:5,price_decimals:4,axis_decimals:2,unit:ut,show_current_marker:!0,show_extremes:!0,show_stats:!0,use_sensor_colors:!0,line_color:"#ffc928",fill_color:"rgba(56, 199, 243, 0.18)",marker_color:"#38c7f3",grid_color:"rgba(255, 255, 255, 0.14)",chart_background:"rgba(33, 52, 62, 0.88)",card_background:"var(--ha-card-background, var(--card-background-color, #1f1f1f))",color_overrides:{},...t}}class Nt extends ct{constructor(){super(...arguments),this._dragging=!1,this._stopPointer=t=>{this._dragging=!1;try{t.currentTarget.releasePointerCapture(t.pointerId)}catch{}}}static async getConfigElement(){return await customElements.whenDefined("real-electricity-price-card-editor"),document.createElement("real-electricity-price-card-editor")}static getStubConfig(){return{type:"custom:real-electricity-price-card",entity:ht,current_price_entity:dt,chart_type:"bar"}}setConfig(t){this._config=Ft(t||{})}getCardSize(){const t=yt(this._config||{});return Math.max(3,Math.ceil((t+150)/50))}render(){if(!this._config)return B``;const t=Ft(this._config),e=t.entity||ht,r=this.hass?.states[e],i=function(t){const e=t?.attributes?.chart_data;return Array.isArray(e)?e.reduce((t,e)=>{const r=e,i=Array.isArray(r)?$t(r[0]):$t(r.x)??$t(r.start_time),s=Number(Array.isArray(r)?r[1]:r.y);return void 0!==i&&Number.isFinite(s)?(t.push({timestamp:i,value:s,color:Array.isArray(r)?void 0:gt(r.fillColor),startTime:Array.isArray(r)?void 0:r.start_time,formattedTime:Array.isArray(r)?void 0:r.formatted_time,formattedPrice:Array.isArray(r)?void 0:r.formatted_price}),t):t},[]).sort((t,e)=>t.timestamp-e.timestamp):[]}(r);if(!r)return this._renderError(`Entity not found: ${e}`);if(!i.length)return this._renderError(`No chart_data found on ${e}`);const s=xt(t),o={width:360,height:yt(t),left:8,right:34,top:16,bottom:27},n=function(t,e){const r=t.map(t=>t.value),i=Math.min(...r),s=Math.max(...r),o=Number(e.min),n=Number(e.max);let a=Number.isFinite(o)?o:Math.min(0,i),c=Number.isFinite(n)?n:s;const l=Math.max(.08*(c-a),.005);return Number.isFinite(o)||(a-=i<0?l:0),Number.isFinite(n)||(c+=l),c<=a&&(c=a+.01),{min:a,max:c}}(i,t),a=function(t,e,r,i){const s=e.height-e.top-e.bottom;return t.map(o=>({...o,x:kt(o.timestamp,t,e,i),y:e.top+(r.max-o.value)/(r.max-r.min)*s}))}(i,o,n,s),c=Math.max(0,Math.min(a.length-1,this._selectedIndex??function(t,e=Date.now()){if(!t.length)return 0;let r=0,i=Math.abs(t[0].timestamp-e);return t.forEach((t,s)=>{const o=Math.abs(t.timestamp-e);o<i&&(r=s,i=o)}),r}(a))),l=a[c],h=a.reduce((t,e)=>e.value<t.value?e:t,a[0]),d=a.reduce((t,e)=>e.value>t.value?e:t,a[0]),p=function(t,e){const r=e.current_price_entity||dt,i=t?.states[r]?.state,s=Number(i);return Number.isFinite(s)?s:void 0}(this.hass,t);return B`
        ...
        <div class="price-content">
          <div class="price-head">
            ...
            <div class="price-selected">
              <span>${Ct(this.hass,l.timestamp)}</span>
              <strong>${wt(l.value,t)}</strong>
            </div>
          </div>

          ${!1===t.show_stats?G:this._renderStats(t,i,h,d,p)}

          <div class="price-chart-frame">
            ${this._renderChart(t,o,a,l,h,d,n,s)}
            <span
              class="price-selected-dot"
              style=${`left:${(l.x/o.width*100).toFixed(2)}%;top:${(l.y/o.height*100).toFixed(2)}%;`}
            ></span>
          </div>
        </div>
      </ha-card>
    ...
    `}_renderChart(t,e,r,i,s,o,n,a){const c=e.height-e.bottom,l=Math.max(e.top,Math.min(c,e.top+(n.max-0)/(n.max-n.min)*(e.height-e.top-e.bottom))),h=St(r),d=e.width-e.left-e.right,p=Math.max(2,Math.min(14,d/Math.max(1,r.length)*.82)),u=function(t){return t.length?Array.from(new Set([0,Math.floor((t.length-1)/3),Math.floor(2*(t.length-1)/3),t.length-1])).filter(e=>e>=0&&e<t.length):[]}(r),m=kt(Date.now(),r,e,a),_=!1!==t.show_current_marker&&m>=e.left&&m<=e.width-e.right,f="rep-line-fill",g="rep-line-color";return B`
      <svg
        class="price-chart"
        ...
        @pointerdown=${t=>this._selectPoint(t,r,e)}
        @pointermove=${t=>this._selectPoint(t,r,e)}
      ...
      >
        <defs>
          ...
          <linearGradient id=${g} x1=${e.left} x2=${e.width-e.right} y1="0" y2="0" gradientUnits="userSpaceOnUse">
            ${r.map((e,i)=>V`
              <stop offset=${i/Math.max(1,r.length-1)*100+"%"} stop-color=${Pt(e,t)}></stop>
            `)}
          </linearGradient>
        </defs>
        ${this._renderGrid(t,e,r,u,n)}
        ${_?V`<line class="price-current-line" x1=${m} x2=${m} y1=${e.top} y2=${c}></line>`:G}
        ${"line"===a?this._renderLineChart(r,c,f,g,t):this._renderBarChart(r,l,p,i,t)}
        ...
        `}
        <line class="price-selected-line" x1=${i.x} x2=${i.x} y1=${e.top} y2=${c}></line>
        <title>${Ct(this.hass,i.timestamp)} ${wt(i.value,t)}</title>
        <desc>${r.length} hourly electricity price points. Inferred interval ${Math.round(h/6e4)} minutes.</desc>
      </svg>
    ...
    `}_selectPoint(t,e,r){if("pointermove"===t.type&&!this._dragging)return;if("pointerdown"===t.type){this._dragging=!0;try{t.currentTarget.setPointerCapture(t.pointerId)}catch{}}t.preventDefault();const i=t.currentTarget.getBoundingClientRect(),s=(t.clientX-i.left)/Math.max(1,i.width)*r.width;let o=0,n=Math.abs(e[0].x-s);e.forEach((t,e)=>{const r=Math.abs(t.x-s);r<n&&(o=e,n=r)}),this._selectedIndex=o}_openMoreInfo(t){t&&_t(this,"hass-more-info",{entityId:t})}}Nt.properties={hass:{attribute:!1},_config:{state:!0},_selectedIndex:{state:!0}},Nt.styles=o`
    :host {
      display: block;
  ...
  `;class Ut extends ct{constructor(){super(...arguments),this._config=Nt.getStubConfig()}setConfig(t){this._config=Ft(t||{})}render(){const t=Ft(this._config);return B`
      <div class="editor">
        ${this._textField("Name","name",t.name||pt)}
        ...
        <label>
          ...
          <select .value=${xt(t)} @change=${t=>this._setValue("chart_type",t.target.value)}>
        ...
        ${this._textField("Unit","unit",t.unit||ut)}
        <div class="grid">
    ...
    `}_setColorOverrides(t){try{const e=JSON.parse(t||"{}");this._setValue("color_overrides",e&&"object"==typeof e?e:{})}catch{}}_setValue(t,e){const r={...this._config};void 0===e||""===e?delete r[t]:r[t]=e,this._config=r,_t(this,"config-changed",{config:r})}}Ut.properties={hass:{attribute:!1},_config:{state:!0}},Ut.styles=o`
    :host {
      display: block;
    }
